                     ...
                     Int_t Nomit=-1)
{
  ...
  for(Int_t pp=0; pp<8; pp++)
  {
    ...
    //sprintf(asym_dist_pat_t[pp],"Run 12 S_{LL} distribution for %s/%s%s",numer,denom,omit_str);
    sprintf(asym_dist_pat_t[pp],"Run 12 S_{LL} distribution for rate-safe %s/%s%s",numer,denom,omit_str);
